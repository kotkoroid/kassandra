// PR-D3e.3: pure-core for the warrior spell system. castSpell() is
// the single entry point called from the event handler; tickSpells()
// advances channelled spells (Rush dash, Hail of Blades spin) every
// frame. No rng usage at the spell layer — outcome math is fully
// deterministic over `world` + inputs + per-level scaling tables.

import { applyDamageToEntityRef } from './combat.ts';
import { emit, genId, playerById } from '../world.ts';
