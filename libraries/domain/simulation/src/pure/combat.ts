// PR-D3e.3: pure-core for damage application + per-kind death effects.
// Everything that reduces an entity's hp routes through
// `applyDamageToEntity` so death side-effects (loot, xp, spider
// split, troller bag drop) happen in one place regardless of source.
// `rollLoot` is the only rng call site here; rng flows through the
// `world.rng.next` callable supplied at the binding point inside
// `onEntityDeath`.
...
import { emit, genId, localPlayer, playerById } from '../world.ts';
...
function onEntityDeath(world: World, index: number, attribution: PlayerId | null) {
  ...
  if (attribution !== null) {
    ...
    const drops = rollLoot(e.monsterId, () => world.rng());
    if (drops.length > 0) {
