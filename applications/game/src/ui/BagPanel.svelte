<script lang="ts">
  ...
  import { getItem, LARS_ID, type ItemId, dispatch, localPlayer } from '@kassandra/simulation-domain-library';
  ...
  const larsCount = $derived(player.lars);
  ...
  const equippedWeapon = $derived(getItem(player.equippedWeaponId));
